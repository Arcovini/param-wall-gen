...
export function getExecutionStateBoundingBox(instance: SolidColumnInstance): Bounds3D {
  return (instance.userData.bounds as ColumnBounds).execution;
}
...
// ----- Key points (local = centroid space) -----

export function getKeyPoints(instance: SolidColumnInstance): ColumnKeyPointsMap {
  console.warn('Column has no keypoints; only centroid is defined.');
  return {} as ColumnKeyPointsMap;
}

export function getKeyPointsWorld(instance: SolidColumnInstance): ColumnKeyPointsMap {
  const localMap = getKeyPoints(instance);
  const placement = (instance.userData.column as ColumnParams).placement;
  const worldMap: Partial<ColumnKeyPointsMap> = {};
  for (const [id, pos] of Object.entries(localMap)) {
    (worldMap as Record<string, Position>)[id] = transformPointByPlacement(pos, placement);
  }
  return worldMap as ColumnKeyPointsMap;
}
